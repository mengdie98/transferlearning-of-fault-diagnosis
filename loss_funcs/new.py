...
class DAAN_LMMD_Loss(AdversarialLoss, MMDLoss, LambdaSheduler):
    def __init__(self, num_class, gamma=1.0, max_iter=1000, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None, x=0.5,
                     **kwargs):
        super(DAAN_LMMD_Loss, self).__init__(gamma=gamma, max_iter=max_iter, kernel_type =kernel_type, kernel_mul=kernel_mul, kernel_num=kernel_num, fix_sigma=fix_sigma, **kwargs)
        # super(MMDLoss, self).__init__(gamma, max_iter, **kwargs)
        super(MMDLoss, self)
        self.x=x
        super(AdversarialLoss, self)
        # self.domain_classifier = self.domain_classifier
    ...
    def forward(self, source, target, source_label, target_logits, source_logits=None):
        lamb = self.lamb()
        self.step()
